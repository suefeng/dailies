...
class rootRSSParserClass {
  ...
  function endElement($parser, $tagName) {
    ...
    elseif ((($tagName == "ITEM") && ($this->feedTYPE != "FEE")) || (($tagName == "ENTRY") && ($this->feedTYPE == "FEE"))) {
      ...
      elseif ($this->operateAs == "dailies") {
        if (($useUniq = FeedForAll_dailies_UseUniqueLink($this->currentItem->title, $this->currentItem->description, $this->currentItem->link, $this->currentItem->guid)) != -1) {
          if ($useUniq == 0) {
            if (isset($debugLevel) && ($debugLevel >= 2)) {
              echo "DIAG: FeedForAll_dailies_UseUniqueLink() => 0, Not using<br>\n";
            }
      ...
      if ($this->UseItem) {
        ...
        if ($this->operateAs == "dailies") {
          //
          // Escape any links
          $this->currentItem->link = FeedForAll_dailies_EscapeLink($this->currentItem->link);
          $this->currentItem->guid = FeedForAll_dailies_EscapeLink($this->currentItem->guid);
          $this->currentItem->creativeCommons = FeedForAll_dailies_EscapeLink($this->currentItem->creativeCommons);
          $this->currentItem->sourceURL = FeedForAll_dailies_EscapeLink($this->currentItem->sourceURL);
          $this->currentItem->enclosureURL = FeedForAll_dailies_EscapeLink($this->currentItem->enclosureURL);
          $this->currentItem->comments = FeedForAll_dailies_EscapeLink($this->currentItem->comments);
          $this->currentItem->rssMeshFeedImageUrl = FeedForAll_dailies_EscapeLink($this->currentItem->rssMeshFeedImageUrl);
          $this->currentItem->rssMeshFeedImageLink = FeedForAll_dailies_EscapeLink($this->currentItem->rssMeshFeedImageLink);
        }
    ...
    elseif (($tagName == "IMAGE") && ($this->insideChannelImage)) {
      ...
      if ($this->operateAs == "dailies") {
        //
        // Escape any links
        $this->FeedImageURL = FeedForAll_dailies_EscapeLink($this->FeedImageURL);
        $this->FeedImageLink = FeedForAll_dailies_EscapeLink($this->FeedImageLink);
      }
      if (FeedForAll_parseExtensions() === TRUE) {
        FeedForAll_parseExtensions_endElemend($parser, $this, $tagName);
      }
    ...
    elseif ((($tagName == "CHANNEL") && ($this->feedTYPE != "FEE")) || (($tagName == "FEED") && ($this->feedTYPE == "FEE"))) {
      ...
      if ($this->operateAs == "dailies") {
        //
        // Escape any links
        $this->FeedLink = FeedForAll_dailies_EscapeLink($this->FeedLink);
        $this->FeedCreativeCommons = FeedForAll_dailies_EscapeLink($this->FeedCreativeCommons);
      }
      if (FeedForAll_parseExtensions() === TRUE) {
        FeedForAll_parseExtensions_endElemend($parser, $this, $tagName);
      }
