...
$XMLfilenames[0] = "http://suefeng.net/demos/dailies/431367_2.xml";
$XMLfilenames[1] = "http://suefeng.net/demos/dailies/431367_3.xml";
...
// and type of files (local or remote) can be used with dailies.php
...
//
// Destination Encoding:  By default dailies.php converts all feeds to UTF-8
...
// CAUTION enabling this will cause dailies.php to fail silently with
...
<head>
	<meta charset="UTF-8">
	<title>Dailies Demo | Sue Feng Design</title>
	<link rel="stylesheet" type="text/css" href="style.css" />
	<link rel="stylesheet" type="text/css" href="default.css" />
</head>
<body>
<div class="suefeng-demo-navi">
		<a href="http://suefeng.net/?p=16"><strong>&laquo; Previous Demo: </strong>Hidden Layers</a>
		<span class="right">
			<a href="http://suefeng.net/?p=17"><strong>Back to Creating a RSS Feed Reader for WordPress Article</strong></a>
		</span>
</div>
';
echo '<div id="content">
<h1>RSS Feed Reader for Wordpress</h1>';
...
@include("FeedForAll_dailies_pro.php");

if (function_exists("FeedForAll_dailies_pro") === FALSE) {
  Function FeedForAll_dailies_pro($source) {
    //
...
if (function_exists("FeedForAll_dailies_limitLength") === FALSE) {
  Function FeedForAll_dailies_limitLength($initialValue, $limit = 0) {
...
if (function_exists("FeedForAll_dailies_sizeToString") === FALSE) {
  Function FeedForAll_dailies_sizeToString($filesize) {
    if ($filesize == "") {
      return "";
    }
...
if (function_exists("FeedForAll_dailies_isTemplate") === FALSE) {
  Function FeedForAll_dailies_isTemplate($templateData) {
...
if (function_exists("FeedForAll_dailies_validExtension") === FALSE) {
  Function FeedForAll_dailies_validExtension($filename, $extensions) {
    $foundValid = FALSE;
...
if (function_exists("FeedForAll_dailies_str_replace") === FALSE) {
  Function FeedForAll_dailies_str_replace($search, $replace, $subject) {
    return str_replace($search, $replace, $subject);
  }
}

if (function_exists("FeedForAll_dailies_encodeURL") === FALSE) {
  Function FeedForAll_dailies_encodeURL($URLstring, $includePND = 0) {
    $result = "";
...
if (function_exists("FeedForAll_dailies_CreateUniqueLink") === FALSE) {
...
if (function_exists("FeedForAll_dailies_UseUniqueLink") === FALSE) {
  Function FeedForAll_dailies_UseUniqueLink($title, $description, $link, $guid) {
    if ($title);
    if ($description);
    if ($link);
    if ($guid);
...
if (function_exists("FeedForAll_dailies_AddIdentity") === FALSE) {
  Function FeedForAll_dailies_AddIdentity($itemString) {
    return "<!-- HTML generated from an RSS Feed by dailies.php, http://www.FeedForAll.com/ a NotePage, Inc. product (http://www.notepage.com/) -->".$itemString;
  }
}
...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (isset($_REQUEST["XMLFILE"])) {
    ...
    elseif (stristr($_REQUEST["XMLFILE"], "://")) {
      ...
      if (($fileAccessLevel == 1) || ($fileAccessLevel == -1)) {
        if (FeedForAll_dailies_validExtension(basename($_REQUEST["XMLFILE"]), $allowedFeedExtensions) === FALSE) {
  ...
  if (isset($_REQUEST["TEMPLATE"])) {
    ...
    elseif (stristr($_REQUEST["TEMPLATE"], "://")) {
      ...
      if (($fileAccessLevel == 1) || ($fileAccessLevel == -1)) {
        if (FeedForAll_dailies_validExtension(basename($_REQUEST["TEMPLATE"]), $allowedTemplateExtensions) === FALSE) {
  ...
  if (isset($outputCacheTTL) && function_exists("FeedForAll_scripts_readOutputCacheFile") && (($cacheContents = FeedForAll_scripts_readOutputCacheFile($XMLfilename, $TEMPLATEfilename)) !== FALSE)) {
    ...
    if (FeedForAll_dailies_isTemplate($template) === FALSE) {
      if (!isset($hideErrors)) {
        echo "$TEMPLATEfilename is not a valid dailies.php template file, exiting\n";
      }
      return;
    }
    ...
    $rss_parser = new baseParserClass("dailies");
    ...
    if ($parseResult == 0) {
      ...
      $template = FeedForAll_dailies_str_replace("~~~FeedXMLFilename~~~", FeedForAll_dailies_EscapeLink($XMLfilename), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedTitle~~~", FeedForAll_dailies_limitLength($rss_parser->FeedTitle, $limitFeedTitleLength), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedDescription~~~", FeedForAll_dailies_limitLength($rss_parser->FeedDescription, $limitFeedDescriptionLength), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedContentEncoded~~~", $rss_parser->FeedContentEncoded, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedLink~~~", FeedForAll_dailies_EscapeLink($rss_parser->FeedLink), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedPubDate~~~", $rss_parser->FeedPubDate, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedPubLongDate~~~", date($LongDateFormat, $rss_parser->FeedPubDate_t), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedPubShortDate~~~", date($ShortDateFormat, $rss_parser->FeedPubDate_t), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedPubLongTime~~~", date($LongTimeFormat, $rss_parser->FeedPubDate_t), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedPubShortTime~~~", date($ShortTimeFormat, $rss_parser->FeedPubDate_t), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageUrl~~~", FeedForAll_dailies_EscapeLink($rss_parser->FeedImageURL), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageTitle~~~", $rss_parser->FeedImageTitle, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageLink~~~", FeedForAll_dailies_EscapeLink($rss_parser->FeedImageLink), $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageDescription~~~", $rss_parser->FeedImageDescription, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageHeight~~~", $rss_parser->FeedImageWidth, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedImageWidth~~~", $rss_parser->FeedImageWidth, $template);
      $template = FeedForAll_dailies_str_replace("~~~FeedCreativeCommons~~~", FeedForAll_dailies_EscapeLink($rss_parser->FeedCreativeCommons), $template);
      ...
      if (!function_exists("FeedForALL_dailies_replaceInItem")) {
        Function FeedForALL_dailies_replaceInItem($source, $currentItem) {
          ...
          $item = FeedForAll_dailies_str_replace("~~~ItemTitle~~~", FeedForAll_dailies_limitLength($currentItem->title, $limitItemTitleLength), $source);
          $item = FeedForAll_dailies_str_replace("~~~ItemDescription~~~", FeedForAll_dailies_limitLength($currentItem->description, $limitItemDescriptionLength), $item);
          $item = FeedForAll_dailies_str_replace("~~~ItemEnclosureLengthFormatted~~~", FeedForAll_dailies_sizeToString($currentItem->enclosureLength), $item);
          $item = FeedForAll_dailies_str_replace("~~~ItemPubLongDate~~~", date($LongDateFormat, $currentItem->pubDate_t), $item);
          $item = FeedForAll_dailies_str_replace("~~~ItemPubShortDate~~~", date($ShortDateFormat, $currentItem->pubDate_t), $item);
          $item = FeedForAll_dailies_str_replace("~~~ItemPubLongTime~~~", date($LongTimeFormat, $currentItem->pubDate_t), $item);
          $item = FeedForAll_dailies_str_replace("~~~ItemPubShortTime~~~", date($ShortTimeFormat, $currentItem->pubDate_t), $item);

          $knownFields = $currentItem->getArrayOfFields();
          foreach ($knownFields as $field) {
            $item = FeedForAll_dailies_str_replace($field, $currentItem->getValueOf($field), $item);
          }

          $item = FeedForAll_dailies_CreateUniqueLink($currentItem->title, $currentItem->description, $currentItem->link, $currentItem->guid, $XMLfilename, $item);
          if (FeedForAll_parseExtensions() === TRUE) {
            $item = FeedForAll_parseExtensions_replaceInItem($currentItem, $item);
          }
          return FeedForAll_dailies_AddIdentity($item);
        }
      }

      //
      // Allow access to the number of times that will be processed in the feed
      $template = FeedForAll_dailies_str_replace("~~~NumberOfFeedItems~~~", min(abs($FeedMaxItems), count($rss_parser->Items)), $template);
          ...
          if (($parts = explode("~~~EndItemsRecord~~~", $parts[1])) !== FALSE) {
            ...
            if ($FeedMaxItems < 0) {
              for ($x = count($rss_parser->Items)-1; $x >= count($rss_parser->Items) + $FeedMaxItems; $x--) {
                $allitems .= FeedForALL_dailies_replaceInItem($block1, $rss_parser->Items[$x]);
                $x--;
                if ($x >= count($rss_parser->Items) + $FeedMaxItems) {
                  //
                  // This is at least one more item so use the Alternate definition
                  //
                  $allitems .= FeedForALL_dailies_replaceInItem($block2, $rss_parser->Items[$x]);
                }
              }
            } else {
      ...
      $resultHTML = FeedForAll_dailies_pro($template);
      echo $resultHTML;
  ...
  echo "<html><head><title>dailies.php URL tool</title><meta http-equiv=\"content-type\" content=\"text/html;charset=$destinationEncoding\"></head><body bgcolor=\"#EEEEFF\">\n";
  //
  // We are in "buildURL" mode to help create properly encoded URLs to pass to dailies.php
  ...
  echo "<center><h1>dailies.PHP LINK TOOL</h1></center>\n";
  echo "<p>To assist with the with the creation of properly encoded URLs for use with dailies.php this tool has been created.  Fill in the URLs or file paths for both the XML file and your template file in the boxes below and then click &quot;Submit&quot;.  The program will then return the URLs properly encoded in a string that calls dailies.php.  You can click on this link to test the results.  The program will also indicate if it was unable to open either of the URLs it was given.</p>\n";
  ...
  if (isset($_POST["submit"])) {
    ...
    if (isset($_POST["XML"]) && $_POST["XML"] != "") {
      $answer .= "XMLFILE=".FeedForAll_dailies_encodeURL($_POST["XML"]);
      $answerAlt .= "\$XMLFILE = \"".str_replace("&", "&amp;", $_POST["XML"])."\";<br>";
      $ssi .= "XMLFILE=".FeedForAll_dailies_encodeURL($_POST["XML"]);
      $xmlurl = FeedForAll_dailies_encodeURL($_POST["XML"]);
    }
    if ((isset($_POST["XML"]) && $_POST["XML"] != "") && (isset($_POST["TEMPLATE"]) && $_POST["TEMPLATE"] != "")) {
      $answer .=  "&amp;";
      $ssi .=  "&amp;";
    ...
    if (isset($_POST["TEMPLATE"]) && $_POST["TEMPLATE"] != "") {
      $answer .=  "TEMPLATE=".FeedForAll_dailies_encodeURL($_POST["TEMPLATE"]);
      $answerAlt .= "\$TEMPLATE = \"".str_replace("&", "&amp;", $_POST["TEMPLATE"])."\";<br>";
      $ssi .=  "TEMPLATE=".FeedForAll_dailies_encodeURL($_POST["TEMPLATE"]);
      $templateurl = FeedForAll_dailies_encodeURL($_POST["TEMPLATE"]);
    }
    ...
    if (isset($_POST["XML"]) && $_POST["XML"] != "") {
      ...
      elseif (stristr($_POST["XML"], "://")) {
        ...
        if (($fileAccessLevel == 1) || ($fileAccessLevel == -1)) {
          if (FeedForAll_dailies_validExtension(basename($_POST["XML"]), $allowedFeedExtensions) === FALSE) {
    ...
    if (isset($_POST["TEMPLATE"]) && $_POST["TEMPLATE"] != "") {
      ...
      elseif (stristr($_POST["TEMPLATE"], "://")) {
        ...
        if (($fileAccessLevel == 1) || ($fileAccessLevel == -1)) {
          if (FeedForAll_dailies_validExtension(basename($_POST["TEMPLATE"]), $allowedTemplateExtensions) === FALSE) {
      ...
      if ($TEMPLATEfilename != "") {
        ...
        elseif (FeedForAll_dailies_isTemplate($templateContents) === FALSE) {
          echo "$_POST[TEMPLATE] is not a valid dailies.php template file\n";
          $templateContents = "";
        } else {
